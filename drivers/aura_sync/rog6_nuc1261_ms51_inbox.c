...
static int asus_usb_hid_write_aprom(u8 addr, u8 *cmd, int cmd_len)
{
	...
	if((g_pd_fw_updating) && (cmd[0]!=0x22 && cmd[0]!=0x24 && cmd[0]!=0x25 && cmd[0]!=0x26)){
		printk("[ROG6_INBOX] skip command 0x%x when pd updating",cmd[0]);
		return ret;
	}
	if((g_ms51_updating) && (addr != 0x51 && cmd[0] != 0xCB && cmd[0] != 0xCA && cmd[0] != 0x60)){
		printk("[ROG6_INBOX_COMMAND] skip command 0x%x when aura updating",cmd[0],cmd[1]);
		return ret;
	}
	...
	if(addr == i2c_addr_select(addr_0x18))
		msleep(g_other_delay);
	else
		msleep(g_mcu_delay);
...
static int WT6639F_firmware_check(struct device *dev)
{
	...
	buffer = kzalloc(FEATURE_READ_COMMAND_SIZE, GFP_KERNEL);
	msleep(10);
	do{
...
err_free:
	...
	kfree(buffer);
	g_pd_fw_updating = 0;
	printk("[ROG6_INBOX]%s--- last addr =  %d",__func__, addr-buffer_size);
	return ret;
}
...
/*
	for (retry = 1; retry <= 10; retry++) {
		...
		val = data[1];
		if(val == 1) {
			printk("[ROG6_INBOX][%s] MS51 is already in AP, %d\n", __func__, val);
			break;
		}
		msleep(10);

		if(retry == 10){
			printk("[ROG6_INBOX][%s] Over retry times %s, Not sure MS51 in AP, %d\n", __func__, retry, val);
			ld_addr = addr_0x16;
			return ld_addr;
		}
	}
...
static ssize_t ms51_fw_ver_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if(g_aura_lpm == 1){
		...
		printk("[ROG6_INBOX] wakeup aura from LPM\n");
	}
	msleep(10);
	memset(cmd, 0, NUC1261_CMD_LEN);
	cmd[0] = 0xCB;
	...
	if(g_aura_lpm == 1){
		...
		ret = asus_usb_hid_write_aprom(i2c_addr_select(nuc1261), cmd, NUC1261_CMD_LEN);
		printk("[ROG6_INBOX] aura enter LPM\n");
	}
...
static int fan_pwm(u8 pwm)
{
	...
	//keep MCU awake if Fan Enable
	if(pwm > 0 && g_fan_cnt == 0){
		usb_autopm_get_interface(to_usb_interface(rog6_inbox_hidraw->hid->dev.parent));
		g_fan_cnt++;
	}else if(pwm <= 0 && g_fan_cnt == 1){
		usb_autopm_put_interface(to_usb_interface(rog6_inbox_hidraw->hid->dev.parent));
		g_fan_cnt--;
	}
	...
	if (err < 0)
		...
		printk("[ROG6_INBOX] FAN PWM: %d, g_fan_cnt=%d\n", pwm,g_fan_cnt);
	mutex_unlock(&pd_lock);
	return err;
}
...
static ssize_t temperature_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if (g_thermometer == THERMOMETER_SHT4x) {
		...
		if(report_the_last_temp && g_last_temperature != -255){
			printk("[ROG6_INBOX] return old temp %d\n", g_last_temperature);
...
static int humidity_correction(int humidity, int temperature)
{
	...
	do{
		...
		//printk("[ROG6_INBOX][%s] %02x, %02x\n", __func__, raw_ambien[0], raw_ambien[1]);
	...
	if(humidity_corrected<0 || humidity_corrected>100){
		printk("[ROG6_INBOX] humidity out of range, report hmidity without calibration, ambient_temp=%d temp=%d humidity=%d",amb_temp,temperature,humidity);
...
static ssize_t delay_debug_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int mcu_delay = 0;
	int other_delay = 0;
	sscanf(buf, "%d,%d", &mcu_delay, &other_delay);
	g_mcu_delay = mcu_delay;
	g_other_delay = other_delay;
	printk("[ROG6_INBOX] set cmd delay, mcu=%d other=%d",g_mcu_delay,g_other_delay);
	return count;
}
...
static ssize_t aura_lpm_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (err < 0)
		...
		printk("[ROG6_INBOX][%s] set low power mode %d\n", __func__, lpm_mode);
		g_aura_lpm = lpm_mode;
	}
	return count;
}
...
static DEVICE_ATTR(delay_debug_mode, 0664, NULL, delay_debug_mode_store);
...
static struct attribute *pwm_attrs[] = {
	...
	&dev_attr_delay_debug_mode.attr,
	NULL
};
...
// Set global variable
	...
	g_ms51_updating = 0;
	g_aura_lpm = -1;
	g_mcu_delay = NUC1261_CMD_DELAY;
	g_other_delay = NUC1261_CMD_DELAY;
...
static void rog6_inbox_usb_remove(struct hid_device *hdev)
{
	...
	if(g_fan_cnt > 0){
		printk("[ROG6_INBOX] autopm put interface, cause Fan enable");
		usb_autopm_put_interface(to_usb_interface(rog6_inbox_hidraw->hid->dev.parent));
		g_fan_cnt--;
	}
#if defined ASUS_AI2201_PROJECT || defined ASUS_AI2202_PROJECT
