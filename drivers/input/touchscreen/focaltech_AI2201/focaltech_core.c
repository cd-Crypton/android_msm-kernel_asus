...
static int fts_gpio_configure(struct fts_ts_data *data)
{
    ...
    /* request reset gpio */
    if (vddio_flag && gpio_is_valid(data->pdata->vddio)) {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    pdata->vddio = of_get_named_gpio_flags(np, "focaltech,vddio",
                      0, &pdata->vddio_flags);
    if (pdata->vddio >= 0) {
        FTS_ERROR("Get vddio %d",pdata->vddio);
	FTS_INFO("[GPIO]vddio is exist, device is before SR1.");
	vddio_flag=true;
    } else if (pdata->vddio < 0)
	vddio_flag=false;
    ...
    if(vddio_flag)
	FTS_INFO("max touch number:%d, irq gpio:%d, reset gpio:%d, vddio:%u",
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    ...
    if (vddio_flag && gpio_is_valid(ts_data->pdata->vddio))
