...
extern int qti_cell1_voltage;  //POWER_SUPPLY_PROP_HEALTH
extern int qti_cell2_voltage;  //POWER_SUPPLY_PROP_HEALTH
extern int qti_user_soc;
extern bool g_vbus_plug;
extern void get_vbus_status_from_ADSP(void);
//ASUS_BSP ---
...
//ASUS_BSP +++ LiJen: speed up to show charger icon when plug-out
	if(pst->prop[USB_ONLINE] && !g_vbus_plug){
		get_vbus_status_from_ADSP();
		if(!g_vbus_plug){
			write_property_id(bcdev, pst, USB_ONLINE, 0);
			write_property_id(bcdev, pst, USB_ADAP_TYPE, 0);
			read_property_id(bcdev, pst, USB_ONLINE);
			read_property_id(bcdev, pst, USB_ADAP_TYPE);
			pr_debug("update usb_adap_type: %u, usb_online=%d\n", pst->prop[USB_ADAP_TYPE], pst->prop[USB_ONLINE]);
		}
	}
//ASUS_BSP ---
		...
		union power_supply_propval *pval)
{
	...
	if (prop == POWER_SUPPLY_PROP_USB_TYPE) {
		if (g_SWITCH_LEVEL == 5 || g_SWITCH_LEVEL == 7){
...
//ASUS_BSP ---
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		//pr_err("[BAT][CHG]%s qti, qti_user_soc=%d\n", __func__, qti_user_soc);
		//pr_err("[BAT][CHG]%s qti, user_soc=%d\n", __func__, pval->intval);		
...
//ASUS_BSP +++
	...
	case POWER_SUPPLY_PROP_HEALTH:  
		//pr_err("[BAT][CHG] qit_battery_charger, Vcell1:%dmV, Vcell2:%dmV\n", qti_cell1_voltage,qti_cell2_voltage);		
		if (qti_cell1_voltage < 3300 || qti_cell2_voltage < 3300)  
		{							//health_type[bat_health])  
			pval->intval = 4;   	//"Over voltage" = 4  => bat_health=4;
			pr_err("[BAT][CHG] Over voltage, Vcell1:%dmV, Vcell2:%dmV\n", qti_cell1_voltage,qti_cell2_voltage);		
		}
		else
			pval->intval = pst->prop[prop_id];	
		break;
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	if (!bcdev){
		pr_err("%s. devm_kzalloc debug failed part 1\n", __func__);
		return -ENOMEM;}
	...
	for (i = 0; i < PSY_TYPE_MAX; i++) {
		...
		if (!bcdev->psy_list[i].prop){
			pr_err("%s. devm_kzalloc debug failed part 2\n", __func__);
			return -ENOMEM;}
	}
	...
	if (!bcdev->psy_list[PSY_TYPE_BATTERY].model){
		pr_err("%s. devm_kzalloc debug failed part 3\n", __func__);
		return -ENOMEM;}
	...
	if (IS_ERR(bcdev->client)) {
		pr_err("%s. debug failed part 4\n", __func__);
		rc = PTR_ERR(bcdev->client);
		if (rc != -EPROBE_DEFER){
			pr_err("%s. debug failed part 5\n", __func__);		
